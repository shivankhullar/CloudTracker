...
std::string get_cloud_name(int i, Params &params)
{
        ...
        else { cloud_name = params.cloud_prefix + std::to_string(i); std::cout << "Make sure to modify the get_cloud_name() function correctly." << std::endl;}
        return cloud_name;
}
...
{
        for (int i=0; i<num_clouds_parent_snap; i++)
        {
                std::string cloud_name = get_cloud_name(i, params);
                // initiate a cloud instance here through read_cloud_data and then pass data from that like pIDs or masses around
                //read_cloud_property(snap_num, params, cloud_name, params.key_particle_IDs);
                //read_cloud_property(snap_num, params, cloud_name, params.key_masses);
